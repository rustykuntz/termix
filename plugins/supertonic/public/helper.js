...
export { chunkText, chunkTextRanges } from './text-chunks.js';
import { chunkText } from './text-chunks.js';
...
export class TextToSpeech {
    ...
    async _infer(textList, langList, style, totalStep, speed = 1.05, progressCallback = null, maxDuration = Infinity) {
        ...
        if (duration.some(value => value > maxDuration)) {
    ...
    async call(text, lang, style, totalStep, speed = 1.05, silenceDuration = 0.3, progressCallback = null, maxSamples = Infinity) {
        ...
        for (let i = 0; i < textList.length; i++) {
            const silenceLen = wavParts.length ? Math.floor(silenceDuration * this.sampleRate) : 0;
            const remainingDuration = Number.isFinite(maxSamples)
                ? Math.max(0, maxSamples - sampleCount - silenceLen) / this.sampleRate
                : Infinity;
            const { wav, duration } = await this._infer(
                [textList[i]], [langList[i]], style, totalStep, speed,
                progressCallback, remainingDuration
            );
            const part = Float32Array.from(wav);
            if (sampleCount + silenceLen + part.length > maxSamples) {
                throw new Error('Speech is too long for the CliDeck audio player.');
            }
            if (silenceLen) wavParts.push(new Float32Array(silenceLen));
            wavParts.push(part);
            sampleCount += silenceLen + part.length;
            durCat += duration[0] + (silenceLen ? silenceDuration : 0);
        }

        const wavCat = new Float32Array(sampleCount);
        let offset = 0;
        for (const part of wavParts) { wavCat.set(part, offset); offset += part.length; }
 ...
 */
export async function loadCfgs(onnxDir, path = `${onnxDir}/tts.json`) {
 ...
 * Load text processor
 */
export async function loadTextProcessor(onnxDir, path = `${onnxDir}/unicode_indexer.json`) {
    const response = await fetch(path);
 ...
 */
export async function loadTextToSpeech(onnxDir, sessionOptions = {}, progressCallback = null, assets = {}) {
    console.log('Using WebAssembly/WebGPU for inference');

    const cfgs = await loadCfgs(onnxDir, assets['tts.json']);

    const dpPath = assets['duration_predictor.onnx'] || `${onnxDir}/duration_predictor.onnx`;
    const textEncPath = assets['text_encoder.onnx'] || `${onnxDir}/text_encoder.onnx`;
    const vectorEstPath = assets['vector_estimator.onnx'] || `${onnxDir}/vector_estimator.onnx`;
    const vocoderPath = assets['vocoder.onnx'] || `${onnxDir}/vocoder.onnx`;
    ...
    const textProcessor = await loadTextProcessor(onnxDir, assets['unicode_indexer.json']);
